...
{   present.innerHTML="";describe.innerHTML="";question.innerHTML="";ans_options.innerHTML="";
    console.log(typeof(qa_list),typeof(qa_list))
    try {var source=presentation_config.content_filepath+qa_list.item[langindex].question[qi].src;}//console.log(source);} 
    catch{}
    console.log(source,langindex);
    console.log((qa_list))
    ...
    {   case 'img': {   var p= document.createElement('img');p.class="img-fluid";p.Id='image_view';p.name='image_view'; ; p.src= source;present.appendChild(p); break;}
        ...
        case 'i360' :{  var p= document.createElement('a');var linkText = document.createTextNode("Open Image");p.appendChild(linkText);p.href = img360; window.open(p,"_blank"); p.Id='i360_view';p.name='audio_view'; p.controls = 'controls'; p.src=source; present.appendChild(p); break;}
