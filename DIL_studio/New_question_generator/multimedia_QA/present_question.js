...
var qi=0;var nq= qa_list2.item[langindex].question.length;

function present_question_mode(mode){
    if(mode=="single_question"){present_question(qi,JSON.parse(localStorage.getItem("preview_data")));/*console.log(qa_list)*/}
    else if(mode=="multiple_question"){present_question(qi,qa_list);/*console.log(qa_list)*/}
    else if(mode=="normal"){present_question(qi,qa_list1);/*console.log(qa_list)*/}
}

function present_question(qi) 
{   //var qa_list=data_list
    console.log("nq:",nq);
    if (localStorage.getItem("mode")=="1"){qa_list=JSON.parse(localStorage.getItem("preview_q_data"));}
    if (localStorage.getItem("mode")=="2"){qa_list=JSON.parse(localStorage.getItem("preview_ql_data"));}
    //qa_list=JSON.parse(localStorage.g"etItem("preview_q_data"));
   // console.log(typeof(qa_list))
    console.log("-----------",(qa_list),localStorage.getItem("mode"));
    console.log(langindex,qi);
    present.innerHTML="";describe.innerHTML="";question.innerHTML="";ans_options.innerHTML="";
    try{var source=presentation_config.content_filepath+qa_list.item[langindex].question[qi].src;console.log(source);}catch{}
    var desc=qa_list.item[langindex].question[qi].des;
    var ques=qa_list.item[langindex].question[qi].ques;
